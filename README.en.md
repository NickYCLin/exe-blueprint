...
| Large-system overview | Inventory mode, solution/project/component references and file progress; optional C# declarations and cross-project call index | No MSBuild evaluation or package restore; source semantics currently require SDK-style projects with unambiguous source membership |
...
For large source folders, use `--source --inventory`, then add `--source-code` when you need C# type/method declarations and resolvable cross-project calls. For compiled application folders, use `--inventory`. See [large-project analysis](docs/large-project-analysis.md) for scope and reproducible scale checks.

**To read an analysis result**, start with `schemaVersion`, `analysisMode`, `summary` and `warnings`, then inspect `projectGraph`, `sourceCode`, `files`, `dependencies`, `technologies` and `archives`. The current `main` output schema is `0.22`.
