...
- 用堆疊模擬把直線（無分支）方法還原成 C# 陳述式，帶不動的方法保留 IL 註解
...
骨架會放在輸出目錄的 `reconstructed-csharp/`。直線（沒有 if／迴圈）的方法會用堆疊模擬還原成
C# 陳述式；帶分支的方法還原不了，會把原始 IL 放進註解當依據，方法體先用 `NotImplementedException`。
...
- 還原帶分支的方法（if／迴圈／switch），讓骨架長出完整方法體、能直接編譯成多專案 solution（目前只還原直線方法）
