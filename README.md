...
- 列出 .NET assembly 的內嵌 manifest 資源（.resources、WPF BAML、內嵌設定檔或組件），標出用途、位置與大小
...
- 擴充中介模型，補上 UI 與設定（函式、型別、欄位、屬性、事件、呼叫圖、內嵌 manifest 資源清單已完成 .NET 部分；接著要解出 .resources 表的實際鍵值與 WPF BAML 內容）
