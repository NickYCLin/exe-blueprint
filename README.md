...
- 列出 .NET assembly 的內嵌 manifest 資源（.resources、WPF BAML、內嵌設定檔或組件），標出用途、位置與大小；`.resources` 會再列出鍵名、型別及可安全解碼的標準值，`.baml` 會整理檔頭版本與 record 類型數量
...
- 擴充中介模型，補上 UI 與設定（函式、型別、欄位、屬性、事件、呼叫圖、內嵌 manifest 資源清單、`.resources` 標準鍵值及 WPF BAML 結構摘要已完成 .NET 部分；接著要解析 BAML 型別／屬性對照與自訂資源型別）
