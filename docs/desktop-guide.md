...
3. 按「開始分析」。預設會產生繁體中文報告、JSON，以及可產生的 C# 重建專案。大型原始碼可選配 C# 宣告與跨專案呼叫索引；其他語言、Ghidra 與覆寫設定在「進階選項」。

來源、儲存位置與 Ghidra 目錄都可貼上成對雙引號包住的路徑，例如檔案總管「複製路徑」的內容，不必先手動刪除引號。
...
分析與最初的資料夾掃描在背景執行，畫面仍可操作取消。取消會在目前檔案或目錄的讀取告一段落後生效。取消或失敗後會顯示原因，原本的設定仍會保留，方便修改後重試；已產生的部分檔案可能仍留在儲存位置。

分析途中關閉視窗時，程式會先取消分析並等待背景工作清理，再自動關閉。等待期間會顯示狀態，不需要重複按關閉。
