...
`div.un`／`rem.un` 只在兩側型別都能確認屬於同一個 int32、int64 或 native-int stack family 時還原；運算元會先明確轉成 `uint`、`ulong` 或 `nuint`，結果寫回同 family 的 signed／窄型別時再使用 `unchecked` 轉型。型別未知、跨 family 或浮點輸入仍會整個方法 fail closed。
