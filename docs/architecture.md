...
標準 `try/finally` 會依 exception region metadata 的保護區域與 handler 邊界還原，不靠跳轉位置猜測；
跨區塊的區域變數會提升到 `try` 外並依 IL locals init 語意先設成 default。`catch`、filter、fault 或非標準例外區域目前仍會退回反組譯 IL 註解加 `NotImplementedException`。
