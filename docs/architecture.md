...
還原命名空間、型別、欄位、屬性、方法簽章與繼承。方法體由 `ManagedSymbolReader` 的直線 IL 堆疊模擬器
還原：載入、算式、欄位、屬性、方法呼叫、`new`、運算子、`return`、`throw` 都能重建成 C# 陳述式，
auto-property 的隱藏欄位也會還原成屬性名稱。帶分支（if／迴圈／例外處理）的方法目前還原不了，
會退回反組譯 IL 註解加 `NotImplementedException`。
重建採全有或全無：遇到任何分支或不支援的指令就整個方法放棄，寧可不還原也不產出語意錯誤的程式碼。
它證明了「中介模型 → 目標語言」這條路走得通，但帶分支的方法體、巢狀型別、事件與泛型限制都還沒處理，所以不保證可直接編譯。
