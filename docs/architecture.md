...
內嵌 `.json` 設定檔會在 1 MB、32 層、20,000 個節點與 10,000 個 object 欄位的上限內解析。`configuration` 只保存根類型、已走訪欄位數和去重後的欄位路徑（最多 200 筆），絕不保存任何 JSON value；超過上限、巢狀過深或 JSON 無效時會標成 partial／invalid 並附原因。

後續可支援 XML 設定、深化自訂資源型別的靜態摘要、原生呼叫圖或更細的控制流程；新增欄位時會再提升 schema 版本，舊欄位維持相容。
