...
- `files[].code`：.NET 型別、同一 artifact 內可精確連結巢狀 owner 的 TypeDef token、ref-like 關係、泛型名稱、owner domain 與可獨立證明 primary constraint 的 additive 明細、欄位、含 index parameters 的屬性、事件、方法簽章、dispatch 與已還原 body 的 `requiresUnsafeContext` 旗標、入口點、方法層級呼叫圖、manifest 資源（含預序列化自訂資源的 `serialization` 摘要）與各方法反組譯出的 IL
...
後續深化自訂資源型別的靜態摘要、原生呼叫圖或更細的控制流程時會再提升 schema 版本，舊欄位維持相容。
