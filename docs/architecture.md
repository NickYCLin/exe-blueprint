...
骨架 writer 會先完整預檢整批 `GeneratedFile.RelativePath`：只接受有 byte／segment 上限的 portable 相對路徑，拒絕絕對／drive／反斜線、`.`／`..`、跨平台保留名稱、case-insensitive／Unicode-normalized 重複與檔案目錄衝突，並以 full-path containment 再次確認目標仍在指定輸出根目錄。實際寫入前也會重查輸出根目錄本身或其下既有與 dangling 的 symbolic link／reparse point；每個檔案先以同目錄 opaque `CreateNew` 暫存檔寫入，再以 move 取代目標，因此既有 hardlink 不會讓外部 inode 被截斷。這是針對靜止輸出樹的 defense-in-depth，不承諾抵禦另一個程序在檢查與 move 之間同時替換 parent directory；輸出樹不應與不可信 writer 共用。
...
`.resources` 內的鍵值會放在 manifest 資源的 `entries`。字串、數字、布林、字元、日期與時間可轉成文字；一般位元組陣列和 stream 會記錄大小及支援的圖片檔頭摘要。遇到 [`System.Resources.Extensions.DeserializingResourceReader`](https://github.com/dotnet/runtime/blob/v10.0.11/src/libraries/System.Resources.Extensions/src/System/Resources/Extensions/DeserializingResourceReader.cs) 的預序列化資源表時，解析器只讀取其 v2 表格與四種[官方 envelope 代碼](https://github.com/dotnet/runtime/blob/v10.0.11/src/libraries/System.Resources.Extensions/src/System/Resources/Extensions/SerializationFormat.cs)：BinaryFormatter、TypeConverter byte array、TypeConverter string、Activator stream；`serialization` 會保存 `format`、payload 大小、魔術碼辨識出的資料種類及完整性。BinaryFormatter、byte array 與 stream 一律只做位元組摘要，僅 TypeConverter string 會在 strict UTF-8 與長度限制內保留原始文字；不載入資源型別、不建立 TypeConverter／Activator，也不呼叫 BinaryFormatter。標頭、型別名稱、鍵名、項目與宣告型別數量都受固定上限，解析失敗會保留明確錯誤。檔名為 `.baml` 時會另外整理 MSBAML 檔頭版本、record 總數、各類型數量、element／property 使用次數，並用 BAML 檔案自行宣告的 assembly、type、attribute、string 對照表解析使用中的 ID。負值 ID 會依 [dotnet/wpf v10.0.11 的官方內建表](https://github.com/dotnet/wpf/tree/v10.0.11/src/Microsoft.DotNet.Wpf/src/PresentationFramework/System/Windows/Markup/Baml2006) 解析，保留號或超出已知範圍的 ID 則維持數字。BAML element 以有上限的 flat node 清單保存 node ID、parent ID、depth、record start/end byte offset、content/complex parent property、child count 與 property value count，避免惡意深度造成遞迴序列化風險，也為 deferred value 定位保留依據；property value 也帶 element ID，可直接與節點對接。不平衡的 element 或 property scope 不會假裝成完整 tree，而會設定 `elementTreeComplete` 與錯誤原因。直接字串、字串表與型別參照、MarkupExtension argument、converter 輸入字串、custom binary 大小及 deferred StaticResource ID 會放在 `propertyValues`；deferred ResourceDictionary 另會保存 string/type/complex key、相對與絕對 value 範圍、shared flags、對應 element，以及每個 key 自己的 optimized 或 verbose StaticResource 表。complex key 依官方 `KeyElementStart`／`KeyElementEnd` 範圍保存型別、原始值與截斷狀態，只對 `StaticExtension`、`TypeExtension`、`String` 和 `ComponentResourceKey` 產生安全摘要。verbose 形式依官方 [`BamlRecordReader`](https://github.com/dotnet/wpf/blob/v10.0.11/src/Microsoft.DotNet.Wpf/src/PresentationFramework/System/Windows/Markup/BamlRecordReader.cs) 的 start/end 範圍處理，只讀取可安全辨識的 `StaticResourceExtension.ResourceKey` 原始值；record 50 與 record 56 都只用所在 value 的 key-local ID 解參照，不會誤查全域 string/type 表。未知 complex key 型別、未知 verbose resource 型別、無法對應 property scope 的 StaticResourceId 或不完整 record 範圍會設定 `deferredResourcesComplete=false` 和明確原因，不會猜測關係。解析器不載入 WPF assembly、不執行 converter／serializer／`ProvideValue`，也不建立 UI 物件。單一 assembly 最多保留 5,000 筆鍵值、單一文字值最多保留 4,096 字元；BAML 最多掃描 100,000 筆 record、每類最多保留 2,000 個 symbol、每檔最多保留 2,000 個 element、2,000 筆 property value、2,000 筆 deferred resource、2,000 筆 complex key value 與 2,000 筆 deferred StaticResource，每個 metadata 字串最多讀取 8,192 bytes，property value 最多保留 4,096 個字元，截斷狀態和解析錯誤都會明確記錄。
...
PNG／GIF 資源項目另附 `imageHeader`，保留格式、檔頭尺寸與錯誤；PNG 會核對 IHDR CRC。這不是完整影像驗證，也不會解壓像素或讀取中繼資料，範圍見[圖片檔頭摘要](resource-image-headers.md)。

後續可深化自訂資源型別的靜態摘要、原生條件／間接 tail call 或更細的控制流程；新增欄位時會再提升 schema 版本，舊欄位維持相容。
...
用內建的 `ExportFunctions.py` 後置腳本把函式、靜態 CALL 與直接 tail call 匯出成 JSON，再由 `GhidraOutputParser` 解析進 `FileArtifact.NativeCode`。
...
腳本明確指定 `@runtime Jython`，避免新版 Ghidra 預設選到需要不同啟動入口的 PyGhidra。Ghidra 12.1.3 需安裝隨附的 Jython 擴充；設定與 Windows x64 實測見[原生分析驗收](native-acceptance.md)。headless 的 script 錯誤可能仍回傳 exit code 0，因此缺少 `functions.json` 一律視為失敗，並在 1,000 字元的診斷上限內優先保留 `REPORT SCRIPT ERROR` 的原因，不讓後續 stack trace 或匯入成功訊息蓋掉錯誤。

匯出格式 v2 新增 `callGraph.calls`，舊 v1 函式輸出仍可讀取，但呼叫圖是 `null`，不會誤當成沒有呼叫。腳本依 Ghidra 的 [FunctionManager](https://github.com/NationalSecurityAgency/ghidra/blob/master/Ghidra/Framework/SoftwareModeling/src/main/java/ghidra/program/model/listing/FunctionManager.java)、[Instruction](https://github.com/NationalSecurityAgency/ghidra/blob/master/Ghidra/Framework/SoftwareModeling/src/main/java/ghidra/program/model/listing/Instruction.java) 與 [ReferenceManager](https://github.com/NationalSecurityAgency/ghidra/blob/master/Ghidra/Framework/SoftwareModeling/src/main/java/ghidra/program/model/symbol/ReferenceManager.java) API，走訪已保留函式的指令，只處理 CALL flow 與 CALL reference；目標必須精確對到函式入口，不依名稱合併、不沿資料指標或 thunk 猜測、不把落在函式中間的位址當成入口。遞迴呼叫、同名函式、外部函式和同一位置的多個已知目標各自保留，同一來源／位置／目標的重複參照只記一次。找不到目標仍保留呼叫位置，`targetAddress=null`；`isIndirect` 來自指令的 computed flow，已知間接目標不代表執行期目標已全部找到。v2 不含跳躍形式的 tail call。

匯出格式 v3 另加上 `isTailCall`：只有無條件、非 computed 且 [`Instruction.getFallThrough()`](https://ghidra.re/ghidra_docs/api/ghidra/program/model/listing/Instruction.html#getFallThrough()) 為 null 的 JUMP 才進入判定；所有 JUMP 參照必須一致指向已保留的另一個函式入口，且該入口不在 caller 的 [`AddressSetView`](https://ghidra.re/ghidra_docs/api/ghidra/program/model/address/AddressSetView.html) 範圍內。判斷使用 Ghidra 的 [FlowType](https://ghidra.re/ghidra_docs/api/ghidra/program/model/symbol/FlowType.html) 分類，不依助記符猜測；資料參照、未知或相互矛盾的目標、函式中間位址、函式內回邊、重疊 body、條件與間接跳躍都不會產生 tail call。這是根據函式邊界與跳躍形狀的靜態判定，沒有證明 ABI 或堆疊狀態，也不沿 thunk 尋找最終目標。

v3 parser 要求每筆呼叫帶 boolean `isTailCall`，tail call 必須是非遞迴且有已知目標的直接呼叫；同一位置的分類矛盾或 tail call 多目標會拒絕整份輸出。v1／v2 繼續相容，v2 的 `tailCallsAnalyzed=false`，v3 為 true；空清單或資料截斷不會改變此能力旗標，仍須配合 `truncated` 判讀。
...
每個 method body 只會取得一次 `MethodBodyBlock` 並經過一次有界解碼，反組譯、body／constructor 還原與呼叫圖共用同一份結果；配置 raw IL 陣列前會先以零配置 reader 檢查 32 KiB 上限，單一方法最多保留 400 個指令、所有 `switch` 合計最多接受 4,096 個 target。raw IL 超限、未知或保留 opcode、截斷 operand、空 method body，以及跳到方法範圍外或 operand 中間的 branch／switch target，都會把方法標為 `ilTruncated` 並傳播到 `code.truncated`。raw IL 超限時不配置或保留任何部分 IL；解碼階段才發現異常時只保留已完整解碼的有界 IL 前綴。兩者都不提交部分呼叫邊，也不嘗試還原 body 或 constructor；完整解碼但缺少支援的終止路徑則只放棄 body 還原，不誤報為資料截斷。
...
local signature 的 `PINNED` qualifier 會保留為安全 taint，並跨 array／pointer／generic／function-pointer 等複合解碼結果傳遞；只要方法含有 pinned local，一般 C# body 就整體 fail closed，不把 GC pinning 與 lifetime 語意洗成普通 local。這仍是完整但尚未支援的合法 IL，因此保留反組譯且不標為 `ilTruncated`；constructor initializer 若已獨立證實仍可保存，但共用的一般 tail body 不會輸出，而 Nullable initobj 特例也會由 raw canonical local-signature gate 拒絕 pinned 形狀。

方法本體的 `InitLocals` flag 也會跟 IL、local signature 與 exception regions 從同一個 `MethodBodyBlock` 讀取。當 flag 為 false 且存在已解碼 local 時，一般 C# body 會整體 fail closed，避免把可能尚未賦值的 CLI local 誤寫成 C# 的確定初始化變數；合法 IL、呼叫圖與已獨立證實的 constructor initializer 仍保留，且不標為 `ilTruncated`。沒有 local 的 tiny／fat method 不受影響；constructor tail 則共用相同規則。
...
call 與 `newobj` 引數也會重用 typed-target gate，只有已驗 enum／integral 同 stack family 才補 `unchecked` 轉型；MemberRef 指向 canonical constructed declaring TypeSpec 時，會先以該 TypeSpec 的結構化 generic arguments 代入正式參數，closed `Span<byte>`／`Nullable<bool>` 等可得到精確型別。無法對齊 arity 的 legacy rendering 不會成為代入證據；開放 `!n`／`!!n` 只有在 TypeSpec、caller 與實際 argument 的 owner／slot provenance 結構相同時才保留，不靠顯示字串猜成具體型別。一般呼叫也沿用相同 generic owner context，不因已證實的 slot 仍以 `!n` 顯示而整批拒絕。
一般方法體的 MemberRef method／field resolver 會先跑獨立的 canonical signature gate，再交給 framework decoder：要求 blob 精確結束、compressed integer 與 type handle 使用最短編碼、`FieldSig`／parameter／return／ordinary type token 出現在 ECMA-335 指定位置，且 nominal handle 的 arity、local GenericParam domain 與 class／valuetype raw kind 一致。可輸出的 ordinary call 另限 default calling convention、無 explicit-this、沒有 vararg sentinel；TypeRef 偽裝成 C# primitive alias、巢狀 wrapper 洗白與不可信 function-pointer convention 都會遞迴拒絕。parent 只接受 TypeDef、TypeRef，或目前 C# renderer 可安全處理、root 為 ARRAY／GENERICINST／SZARRAY 的 TypeSpec；TypeSpec 內的 `!n`／`!!n` 必須落在已驗 caller type／method generic domain，closed generic parent 則同時替 method 與 field signature 做結構化代入。泛型 TypeDef 的 static member 必須證明是目前 caller owner，並以明確的 `Owner<!0, ...>` 具現化輸出；constructed instance receiver 若和 member parent 使用相同 generic definition，則每個 argument 與 metadata identity 都必須結構相同，同文字但來自不同 assembly 的型別不會互認。巢狀型別的 inherited generic slot 依 owner 與連續 position 驗證，不把合法但不同的參數名稱當成 identity。ARRAY parent 只接受可由 C# 命名的 rank 2 以上 MDARRAY 或 SZARRAY，且 shape、zero lower bounds、`int32` index 與 element signature 全部吻合的 `.ctor`／`Get`／`Set` pseudo-member，分別輸出 C# array allocation／index read／index write；CLR rank-1 `T[*]` 與 `Address` 在現有 renderer／managed-address provenance 尚未支援前 fail closed。MethodSpec 也會在配置與 decode 前驗證 `0x0A` header、最短編碼的 argument count、caller slot domain、ordinary type grammar、64 個 argument 上限與精確 EOF；wrapper 內的不支援 function pointer taint 不會被洗成 `nint` 後通過。合法但目前無法無損輸出的 custom modifier、ByRef、TypedReference 或不支援的 function pointer 會讓該 body 還原 fail closed。此 body gate 的單一 MemberRef／MethodSpec signature 上限為 4 KiB、單一 TypeSpec 256 bytes、展開後 TypeSpec 累計 4 KiB、1,024 個 type node、64 層深度、256 個 parameter、64 個 generic argument、32 維 array、64 個 modifier；名稱也有單段與累計 byte budget，MethodSpec 具現化後的簽章與呼叫圖名稱另受 64 KiB 共用 rendered-character budget，避免惡意 metadata 以巢狀、重複 TypeSpec 或 `!!n` 文字替換放大解析成本。
反組譯與呼叫圖對 MemberRef／MethodSpec target 重用同一個 raw gate；MethodDef target 也會驗證 generic domain 與 static／instance header 一致性。呼叫圖只需保存已證實的 metadata identity，因此合法 ByRef、vararg 或跨 generic caller owner 等「C# body 目前無法表示」的 target 仍保留 edge；只有 raw target、array pseudo-member identity、MethodSpec arity／argument 或 bounded rendered name 無法證實時，該 caller 的暫存 edge batch 才會整批捨棄並把 code 標為 truncated，不留下看似完整的部分呼叫關係。欄位 opcode 必須和已知 FieldDef static／instance flags 相符，Literal 不可寫入，InitOnly 只容許同 owner `.ctor`／`.cctor` 的對應 store。方法體若在 evaluation stack 還保存舊運算結果時先輸出 store、void call 等副作用陳述式，也會整體 fail closed；`pop` 只有在 expression provenance 證明可形成合法 C# method-invocation 或 object-creation statement 時才輸出。在尚未建立 synthetic temporary provenance 前，不把舊 expression text 延後到副作用之後重新求值而改變語意。
